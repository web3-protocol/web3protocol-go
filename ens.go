...
func (client *Client) getAddressFromNameService(nameServiceChain int, nameWithSuffix string) (common.Address, int, error) {
	...
	if len(strings.Split(nameWithSuffix, ".")) == 1 {
		return common.Address{}, 0, &Web3ProtocolError{HttpCode: http.StatusBadRequest, Err: errors.New("Unrecognized domain name")}
	}
...
func (client *Client) resolve(nameServiceChain int, resolver common.Address, nameHash [32]byte) (common.Address, int, error) {
	...
	if err != nil {
		return common.Address{}, 0, &Web3ProtocolError{HttpCode: http.StatusBadRequest, Err: err}
	}
	...
	if common.Bytes2Hex(bs) == EmptyAddress {
		return common.Address{}, 0, &Web3ProtocolError{HttpCode: http.StatusBadRequest, Err: errors.New("Empty address")}
	}
...
func (client *Client) getResolver(nsAddr common.Address, nameHash [32]byte, nameServiceChain int, nameWithSuffix string) (common.Address, error) {
	...
	if err != nil {
		return common.Address{}, &Web3ProtocolError{HttpCode: http.StatusBadRequest, Err: err}
	}
	...
	if common.Bytes2Hex(bs) == EmptyAddress {
		return common.Address{}, &Web3ProtocolError{HttpCode: http.StatusBadRequest, Err: errors.New("Cannot resolve domain name")}
	}
...
func (client *Client) getConfigs(nameServiceChain int, nameWithSuffix string) (DomainNameServiceChainConfig, error) {
	...
	if len(ss) <= 1 {
		return DomainNameServiceChainConfig{}, &Web3ProtocolError{HttpCode: http.StatusBadRequest, Err: errors.New("Invalid domain name: " + nameWithSuffix)}
	}
	suffix := ss[len(ss)-1]
	chainInfo, ok := client.Config.Chains[nameServiceChain]
	if !ok {
		return DomainNameServiceChainConfig{}, &Web3ProtocolError{HttpCode: http.StatusBadRequest, Err: errors.New(fmt.Sprintf("unsupported chain: %v", nameServiceChain))}
	}
	domainNameService := client.Config.GetDomainNameServiceBySuffix(suffix)
	if domainNameService == "" {
		return DomainNameServiceChainConfig{}, &Web3ProtocolError{HttpCode: http.StatusBadRequest, Err: errors.New("Unsupported domain name suffix: " + suffix)}
	}
...
func (client *Client) parseChainSpecificAddress(addr string) (common.Address, int, error) {
	...
	if len(ss) != 2 {
		return common.Address{}, 0, &Web3ProtocolError{HttpCode: http.StatusBadRequest, Err: errors.New("invalid contract address from name service: " + addr)}
	}
	chainName := ss[0]
	chainId := client.Config.GetChainIdByShortName(strings.ToLower(chainName))
	if chainId == 0 {
		return common.Address{}, 0, &Web3ProtocolError{HttpCode: http.StatusBadRequest, Err: errors.New("unsupported chain short name from name service: " + addr)}
	}
